...
* **epubcheck (zip file)** (Java installed is required for run it): https://github.com/IDPF/epubcheck/releases
...
options:
  ...
  -l, --log [DIR]       path to directory to write log file. If DIR is omitted write log to directory with epub files
  -i, --individual [NR]
  ...
  --replace-font-family [old,new]
                        pair of "old_font_family,new_font_family"(only with -e and with --font-dir)
  -a, --alter           alternative output display
  -n, --rename          rename .epub files to 'author - title.epub'
  -t, --prepare-send-to-kindle
  ...
  --fix-missing-container
                        Fix missing META-INF/container.xml file in original EPUB file (only with -e)
  --book-margin [NUMBER]
